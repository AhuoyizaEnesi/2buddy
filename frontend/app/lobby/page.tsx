...
const subjects = [
  ...
  },
  {
    id: "chemistry",
    label: "Chemistry",
    description: "Reactions, equations, periodic table",
    color: "#f97316",
    bg: "#fff7ed",
    border: "#fdba74",
    activeBorder: "#f97316",
...
export default function LobbyPage() {
  ...
  useEffect(() => {
    ...
    socket.on("pair_accepted", async (data: { from_sid: string; accepted_by: string; subject: string }) => {
      setPendingRequest(null);
      showNotification(`${data.accepted_by} accepted your request. Creating session...`);
      try {
        setLoading(true);
        const res = await sessionsAPI.createDirect(data.subject);
        const session = res.data;
        socket.emit("leave_lobby", { subject: data.subject });
        router.push(`/board?session=${session.id}&invite=${data.from_sid}&invite_subject=${data.subject}`);
      } catch {
    ...
    socket.on("pair_confirmed", async (data: { subject: string; session_id: string }) => {
      showNotification("Joining session...");
    });
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      ...
      socket.emit("leave_lobby", { subject: pairRequest.subject });
      setPairRequest(null);
  ...
  const handleSolo = async () => {
    ...
    try {
      ...
      const socket = connectSocket();
      socket.emit("leave_lobby", { subject: selectedSubject });
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f8fafc", display: "flex", flexDirection: "column" }}>
      ...
      {pairRequest && (
        <div style={{
          ...
          <div style={{
            backgroundColor: "#ffffff", padding: 32, width: 380,
            border: "1px solid #e5e7eb", boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
          }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button
                ...
                disabled={loading}
                style={{
                  flex: 1, padding: "10px 0", backgroundColor: "#1e3a5f",
              ...
              <button
                ...
                style={{
                  flex: 1, padding: "10px 0", backgroundColor: "#f3f4f6",
                  color: "#374151", border: "1px solid #e5e7eb", fontSize: 13,
                  fontWeight: 700, cursor: "pointer", borderRadius: 0,
                }}
              >
                Decline
      ...
      <div style={{ flex: 1, maxWidth: 900, margin: "0 auto", padding: "32px 24px", width: "100%" }}>
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#1e3a5f", marginBottom: 4 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  backgroundColor: isSelected ? subject.bg : "#ffffff",
                  border: `2px solid ${isSelected ? subject.activeBorder : "#e5e7eb"}`,
              ...
              >
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: "#1e3a5f", marginBottom: 3 }}>
                  {subject.label}
                </div>
        ...
        {selectedSubject && (
          <div style={{
            ...
            <div style={{
              ...
              <div>
                <span style={{ fontSize: 14, fontWeight: 700, color: "#1e3a5f", textTransform: "capitalize" }}>
              ...
              <button
                ...
                disabled={loading}
                style={{
                  padding: "7px 16px", backgroundColor: "#f3f4f6",
                  ...
                  {otherUsers.map((lobbyUser) => (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{
                          width: 32, height: 32, backgroundColor: selectedSubjectData?.color || "#6b7280",
                          display: "flex", alignItems: "center", justifyContent: "center",
                      ...
                      <button
                        ...
                        style={{
                          padding: "7px 16px",
                          backgroundColor: pendingRequest === lobbyUser.username ? "#f3f4f6" : "#1e3a5f",
                          border: "none", color: pendingRequest === lobbyUser.username ? "#9ca3af" : "#fff",
                          fontSize: 12, fontWeight: 700, cursor: pendingRequest === lobbyUser.username ? "not-allowed" : "pointer",
                          borderRadius: 0,
                        }}
                      >
